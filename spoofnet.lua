...
local function id_as_channel(id, fakeId)
    print(id)
    print(fakeId)
    return (id or fakeId) % MAX_ID_CHANNELS
end
...
function open(modem, fakeId)
    expect(1, modem, "string")
    if peripheral.getType(modem) ~= "modem" then
        error("No such modem: " .. modem, 2)
    end
    peripheral.call(modem, "open", id_as_channel(nil,fakeId))
...
function close(modem, fakeId)
    ...
    if modem then
        ...
        peripheral.call(modem, "close", id_as_channel(nil,fakeId))
...
-- @since 1.31
-- @see rednet.open
function isOpen(modem, fakeId)
    expect(1, modem, "string", "nil")
    if modem then
        -- Check if a specific modem is open
        if peripheral.getType(modem) == "modem" then
            return peripheral.call(modem, "isOpen", id_as_channel(nil,fakeId)) and peripheral.call(modem, "isOpen", CHANNEL_BROADCAST)
        end
    else
...
function send(recipient, message, protocol, fakeId)
    ...
    -- Create the message
    local reply_channel = id_as_channel(nil,fakeId)
    ...
    local sent = false
    if recipient == os.getComputerID() then error("spoofnet confused, cant send to self.")
    else
        -- Send on all open modems, to the target and to repeaters
        if recipient ~= CHANNEL_BROADCAST then
            recipient = id_as_channel(recipient,fakeId)
        end

        for _, modem in ipairs(peripheral.getNames()) do
            if isOpen(modem,fakeId) then
...
function broadcast(message, protocol, fakeId)
    expect(2, protocol, "string", "nil")
    send(CHANNEL_BROADCAST, message, protocol, fakeId)
end
...
function run(fakeId)
    ...
    while true do
        ...
        if event == "modem_message" then
            ...
            if channel == id_as_channel(nil,fakeId) or channel == CHANNEL_BROADCAST then
                if type(message) == "table" and type(message.nMessageID) == "number"
                    ...
                    and ((message.nRecipient and message.nRecipient == fakeId) or channel == CHANNEL_BROADCAST)
                    and isOpen(modem)
                then
